...
def table_insert(table, data, columns=None):
    placeholders = ', '.join(['%s'] * len(data))
    if columns:
        columns_formatted = ', '.join(columns)
        query = f"INSERT INTO {table} ({columns_formatted}) VALUES ({placeholders})"
    else:
...
def table_delete(table, id):
    query = "DELETE FROM {table} WHERE dsid = %s;".format(table=table)
    cursor.execute(query, (id,))
    connection.commit()

def table_update(table, id, column, new_value):
    query = f"UPDATE {table} SET {column} = %s WHERE dsid = %s;"
...
def insert_employee_entry():
    ...
    try:
        ...
        if len(attribute_list) != 11:
            print("Invalid amount of entries! Expected 11 entries.")
            return

        data_types = [str, str, str, str, float, str, float, str, int, str, str]
        employee_data = [convert(attr) for convert, attr in zip(data_types, attribute_list)]
        employee_data.insert(0, dsid)  # Insert dsid at the beginning
        table_insert('employees', tuple(employee_data))
        print("Employee entry successfully added.")
    except ValueError as e:
        print(f"Error in input or data conversion: {e}")
...
def update_employee_salary():
    ...
    try:
        table_update('jobsalaries', employee_id, 'salary', new_salary)
        table_update('jobsalaries', employee_id, 'salary_in_usd', new_salary_in_usd)
